    ...
    <div style={{ maxWidth: 680, margin: '0 auto' }}>
      ...
      <div className="space-y-4">
        <Section title="Quando Isso Acontece?">
          Imagina que você estava bem no torneio, mas foi perdendo aos poucos. Agora você tem <strong style={{ color: '#e5484d' }}>menos de 15 fichas grandes</strong> (big blinds). Nesse momento, o jogo muda completamente — só existem duas opções: ir <strong style={{ color: '#4fce82' }}>all-in</strong> ou <strong style={{ color: '#e5484d' }}>foldar</strong>. Acabou a opção de "dar um raise pequeno e ver o que acontece".
        </Section>
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              {STACKS_OPTIONS.map(s => (
                <div key={s} className="rounded-lg p-2" style={{ background: '#0f0f0f', border: '1px solid #2a2a2e' }}>
                  <div style={{ color: '#e5484d', fontWeight: 700 }}>{s} fichas</div>
...
function Trainer() {
  ...
  function answer(action) {
    ...
    if (isLast) recordSession(2, Math.round((newCorrect / newTotal) * 100))
