...
const SCENARIOS = [
  ...
  () => {
    ...
    return {
      q: `River em board ${ranks.join('-')} rainbow. Você e ${pos.heroPos} (IP) e quer apostar. Qual tipo de range usar?`,
      a: 'Polarizado (apostar com nuts e bluffs, check o meio)',
      b: 'Merged (apostar com tudo que e razoavel)',
      aCorrect: true,
      explanation: `No river IP, range polarizado e ideal: aposta com mãos monstro (dois pares+, sets) e bluffs puros (draws que não fecharam). Checa mãos medianas — elas tem showdown value e não precisam de fold equity. Sizing: 66-100%+.`,
  ...
  () => {
    ...
    return {
      q: `Flop ${ranks.join('-')} rainbow. Você e ${pos.heroPos} (OOP) e o raiser. Qual tipo de range usar para c-bet?`,
      a: 'Merged (apostar com overpairs, top pairs, middle pairs)',
      b: 'Polarizado (so nuts e bluffs)',
      aCorrect: true,
      explanation: `OOP você precisa proteger seu range de check. Em boards secos, merge e melhor: aposta frequente com sizing pequeno (33-50%) cobrindo overpairs ate middle pairs. Polarizar OOP deixa o range de check vulneravel ao vilão IP.`,
      boardCards: makeBoardRainbow(ranks), heroCards: heroHand,
  ...
  () => {
    ...
    return {
      ...
      a: 'Polarizado: nuts + bluffs puros (nada no meio). Merged: tudo que e bom, do melhor ate mãos medianas.',
      b: 'Polarizado: so mãos fortes. Merged: so mãos medianas.',
      aCorrect: true,
      explanation: `Polarizado = "bimodal" — aposta com o melhor OU o pior, nunca o meio. Merged = "linear" — aposta com qualquer mão que tem valor. A ausencia das mãos medianas no range de aposta define o polarizado.`,
  ...
  () => {
    ...
    const heroHand = makeHeroCards(...pick(TOP_PAIRS), false)
    return {
      q: `Board seco ${ranks.join('-')} rainbow. Raiser ${pos.heroPos} (IP) vs ${pos.villainPos}. Qual estratégia de c-bet?`,
      ...
      aCorrect: true,
      explanation: `Board seco = poucos draws = equities estaveis. IP com range advantage, merge e eficiente: aposta pequena com muitas mãos de valor. O vilão não melhora muito em turns secos, entao não precisa de fold equity maxima agora.`,
      boardCards: makeBoardRainbow(ranks), heroCards: heroHand,
      heroPos: pos.heroPos, villainPos: pos.villainPos, villainAction: 'Check', potLabel: pick(POT_LABELS_SRP),
    }
  },
  ...
  () => {
    ...
    return {
      q: `Board umido ${ranks.join('-')} com flush draw. Raiser ${pos.heroPos} (IP). Qual estratégia?`,
      a: 'Polarizado: apostar grande com sets/overpairs fortes e semi-bluffs (draws), check mãos medianas',
      b: 'Merge: apostar com tudo incluindo mãos medianas',
      aCorrect: true,
      explanation: `Board umido = muitos draws = equities volateis. Polarizar IP e correto: aposta grande (66%+) com mãos monstro e semi-bluffs (flush/straight draws). Mãos medianas como segundo par tem showdown value mas não querem enfrentar raise — check.`,
      boardCards: board, heroCards: makeHeroCards('A','A',false),
      heroPos: pos.heroPos, villainPos: pos.villainPos, villainAction: 'Check', potLabel: pick(POT_LABELS_SRP),
    }
  },

  // 6. River IP — bluff com mão sem showdown value
  () => {
    const ranks = pick(DRY_RIVERS)
    const pos = pickPos(IP_POSITIONS)
    const bluffHand = makeHeroCards(...pick(BLUFF_HANDS), true)
    return {
      q: `IP no river apos check-check no turn. Board ${ranks.join('-')}. Você tem draw que não fechou (sem showdown value). Apostar como bluff?`,
      a: 'Sim — mão sem showdown value e candidata ideal a bluff no range polarizado',
      b: 'Não — check back, pode ganhar de alguma mão fraca',
      aCorrect: true,
      explanation: `Mao sem showdown value não ganha no showdown de qualquer forma. Transformar em bluff e lucrativo: você so ganha se o vilão foldar. O check-check no turn cappou o range do vilão — ambiente perfeito para bluff polarizado no river com sizing grande.`,
  ...
  () => {
    ...
    const board = makeBoardConnectedWet(ranks)
    return {
      q: `OOP no flop ${ranks.join('-')} em 3-bet pot. Você e o 3-bettor com range premium. Qual abordagem?`,
      a: 'Merge: c-bet com overpairs e sets, check overcards que não conectaram',
      b: 'Polarizado: apostar so com nuts absolutos e bluffs puros',
      aCorrect: true,
      explanation: `OOP em 3-bet pot, merge e correto mesmo em board conectado. Seus overpairs (AA-JJ) precisam de proteção. Aposte com mãos de valor claras usando sizing médio (50-66%). Não polarize OOP pois seu range de check fica vulneravel.`,
  ...
  // 8. Turn IP — vilão checou duas vezes, polarizar
  () => {
    const ranks = pick(DRY_TURNS)
    const pos = pickPos(IP_POSITIONS)
    const heroHand = makeHeroCards(...pick(TOP_PAIRS), false)
    return {
      q: `Turn ${ranks.join('-')}. ${pos.villainPos} checou flop e turn. Você e ${pos.heroPos} (IP). Qual range para bet?`,
      a: 'Polarizado: apostar com mãos muito fortes (dois pares+) e bluffs puros, check mãos medianas',
      b: 'Merged: apostar com tudo que e razoavel',
      aCorrect: true,
      explanation: `Quando o vilão checa duas streets, o range dele esta fraco e cappado. IP você polariza no turn: aposta grande (75-100%) com nuts e bluffs puros. Mãos medianas ja ganham no showdown contra esse range fraco — não precisam apostar.`,
      boardCards: makeBoardRainbow(ranks), heroCards: heroHand,
      heroPos: pos.heroPos, villainPos: pos.villainPos, villainAction: 'Check', potLabel: pick(POT_LABELS_RIVER),
    }
  },
  ...
  () => {
    ...
    const pos = pickPos(IP_POSITIONS)
    return {
      q: 'Regra geral: em quais situações você tende a polarizar mais?',
      a: 'IP, streets tardias (turn/river), boards umidos/conectados',
      b: 'OOP, flop, boards secos/estaticos',
      aCorrect: true,
      explanation: `Polarizacao e mais eficiente: (1) IP — você ve a reacao do vilão antes de agir; (2) Streets tardias — ranges mais definidos, mão ja esta mais "resolvida"; (3) Boards umidos — equities volateis criam bluffs naturais com draws. OOP prefere merge para proteger range de check.`,
  ...
  () => {
    ...
    return {
      ...
      aCorrect: true,
      explanation: `Merged aposta com muitas mãos de valor — sizing pequeno extrai valor de todas e não precisa de fold equity. Polarizado quer maximizar: mãos fortes querem valor máximo do vilão; bluffs precisam de fold equity alta. Por isso sizing grande (66-100%+).`,
      boardCards: makeBoardRainbow(ranks), heroCards: heroHand,
      heroPos: pos.heroPos, villainPos: pos.villainPos, villainAction: 'Check', potLabel: pick(POT_LABELS_SRP),
    }
  },

  // 11. River IP — check back com mão mediana
  () => {
    const ranks = pick(DRY_RIVERS)
    const pos = pickPos(IP_POSITIONS)
    ...
    return {
      q: `River ${ranks.join('-')}. Você e ${pos.heroPos} (IP) com par médio (${pair[0]}${pair[1]}). Apostar ou check?`,
      a: `Check back — par médio e mão do meio: não e nuts (nao aposta por valor) nem bluff (tem showdown value)`,
      b: 'Apostar — proteger showdown value contra bluffs do vilão',
      aCorrect: true,
      explanation: `No river IP com range polarizado, mãos medianas ficam no range de check. ${pair[0]}${pair[1]} não e forte o suficiente para apostar por valor (muitas mãos melhores chamam), e tem showdown value demais para bluffar. Check back e correto.`,
      boardCards: makeBoardRainbow(ranks), heroCards: heroHand,
      heroPos: pos.heroPos, villainPos: pos.villainPos, villainAction: 'Check', potLabel: pick(POT_LABELS_RIVER),
    }
  },

  // 12. Board seco IP — merge c-bet alta frequência
  () => {
    const ranks = pick(DRY_FLOPS)
    const pos = pickPos(IP_POSITIONS)
    ...
    return {
      q: `Board ${ranks.join('-')} rainbow. ${pos.heroPos} faz c-bet ${freq} do range com sizing ${sizing}. Isso e estratégia:`,
      a: 'Merged (alta frequência, sizing pequeno = muitas mãos de valor apostam)',
      b: 'Polarizada (baixa frequência, sizing grande)',
      aCorrect: true,
      explanation: `C-bet frequente com sizing pequeno = estratégia merged classica. IP com range advantage em board seco, aposta com muitas mãos (overpairs, top pairs, overcards com backdoors, underpairs) usando sizing mínimo. Polarizado seria baixa frequência (40-50%) com sizing grande.`,
  ...
  () => {
    ...
    return {
      ...
      a: 'Range de check fica cappado e vulneravel — vilão IP explora apostando frequentemente',
      b: 'Não tem mãos fortes suficientes para polarizar',
      aCorrect: true,
      explanation: `Se você polariza OOP, seu range de check fica cheio de mãos medianas sem proteção. O vilão IP ve isso e aposta com frequência alta contra esse range fraco/cappado. Por isso OOP prefere merge: distribui mãos boas entre aposta e check, protegendo ambos os ranges.`,
      boardCards: makeBoardRainbow(ranks), heroCards: heroHand,
  ...
  () => {
    ...
    return {
      ...
      a: 'Merge extremo: range advantage enorme — qualquer mão pode representar o trip',
      b: 'Polarizado: so apostas com sets/trips e total air',
      aCorrect: true,
      explanation: `Board pareado alto (${paired[0]}-${paired[0]}) e o extremo do merge. ${pos.heroPos} tem range advantage absurdo — todos os Ax/${paired[0]}x, enquanto ${pos.villainPos} raramente tem trips. C-bet 100% com sizing mínimo funciona porque qualquer mão pode representar o trip, forcando o vilão a foldar muito.`,
  ...
  () => {
    ...
    return {
      q: `Flop monotone (${ranks.join('-')} todo ${fs === 's' ? 'espadas' : fs === 'h' ? 'copas' : fs === 'd' ? 'ouros' : 'paus'}). Você e ${pos.heroPos} (IP). Qual estratégia?`,
  ...
  () => {
    ...
    const pos = pickPos(IP_POSITIONS)
    return {
      q: `Turn ${turn} de naipe diferente em board J-T-8 (tinha flush draw). Draws de straight parcialmente fecharam. ${pos.heroPos} (IP). Qual estratégia?`,
      a: 'Polarizado: sizing grande com mãos muito fortes (sets, dois pares) e bluffs puros (draws mortos)',
      b: 'Merged: apostar com tudo que e razoavel pois o board secou',
      aCorrect: true,
      explanation: `Turn que fecha draws parcialmente e board de turn — mais perto do river, ranges mais definidos. IP o correto e polarizar: sets/dois pares apostam por valor máximo; draws mortos (sem equity) se transformam em bluffs. Mãos medianas como par simples checam — tem showdown value mas não querem enfrentar raise.`,
  ...
  () => {
    ...
    return {
      ...
      a: 'Merge: c-bet ~75% com sizing médio (50-66%), cobrindo toda a faixa de valor',
      b: 'Polarizado: c-bet 40% com sizing 100%, so nuts e bluffs',
      aCorrect: true,
      explanation: `Em 3-bet pot OOP, seus overpairs (AA, KK, QQ) precisam proteção contra overcards. Merge funciona: aposta com mãos de valor (overpairs, top pairs fortes) usando sizing médio. Polarizar OOP deixaria range de check cheio de mãos medianas vulneraveis.`,
      boardCards: makeBoardRainbow(ranks), heroCards: heroHand,
      heroPos: pos.heroPos, villainPos: pos.villainPos, villainAction: 'Call', potLabel: pick(POT_LABELS_3BET),
    }
  },

  // 18. River OOP — mergear frequência de aposta low
  () => {
    const ranks = pick(DRY_RIVERS)
    const pos = pickPos(OOP_POSITIONS)
    const heroHand = makeHeroCards(...pick(TOP_PAIRS), false)
    return {
      q: `River ${ranks.join('-')}. Você e ${pos.heroPos} (OOP) com top pair bom kicker. Apostar ou check?`,
      a: 'Check/call ou bet pequeno — OOP no river merge com sizing pequeno, não polariza com sizing grande',
      b: 'Apostar 100% do pot — polarizar forte para maximizar valor',
      aCorrect: true,
      explanation: `OOP no river, mesmo com mão de valor, a tendencia e usar sizing menor ou check/call. Se apostar, sizing pequeno (33-50%) e merged: value bet contra range amplo do vilão. Sizing grande 100% seria polarizado e expoe a mão a raises com bluffs do vilão IP. OOP e sempre mais conservadora.`,
      boardCards: makeBoardRainbow(ranks), heroCards: heroHand,
      heroPos: pos.heroPos, villainPos: pos.villainPos, villainAction: 'Check', potLabel: pick(POT_LABELS_RIVER),
    }
  },
  ...
  () => {
    ...
    const pos = pickPos(IP_POSITIONS)
    return {
      q: `${pos.heroPos} (IP) tem flush draw em flop ${ranks.join('-')} umido. Apostar como semi-bluff faz parte de qual estratégia?`,
      a: 'Polarizado — draws sao bluffs com equity (semi-bluffs) que compoem o range de aposta polarizado',
      b: 'Merged — draws sao mãos medianas que se encaixam no meio do range',
      aCorrect: true,
      explanation: `Semi-bluffs (draws com muitos outs) sao o "bluff" do range polarizado. Você aposta com eles porque: (1) tem fold equity imediata; (2) se chamado, ainda tem equity para melhorar. No range polarizado: nuts + semi-bluffs = lado de aposta; mãos medianas sem draw = lado de check.`,
  ...
  () => {
    ...
    return {
      q: `Por que a posição (IP vs OOP) muda a estratégia de range de aposta?`,
      a: 'IP polariza mais (ve reacao do vilão, age por último). OOP merga mais (sem info, precisa proteger range de check).',
      b: 'IP merga mais (tem vantagem posicional). OOP polariza mais (precisa de fold equity).',
      aCorrect: true,
      explanation: `IP: você age por último, entao pode polarizar com eficiencia — ve se o vilão checa antes de decidir o sizing. OOP: você age primeiro, não sabe o que o vilão vai fazer. Polarizar OOP deixa o range de check vulneravel. Por isso OOP usa merge para distribuir mãos de valor entre aposta e check, criando dois ranges defensaveis.`,
  ...
  () => {
    ...
    const pos = pickPos(IP_POSITIONS)
    return {
      q: `River em J-T-8-2-3 (flush draw não fechou). ${pos.heroPos} (IP) tem Ax do mesmo naipe do board (draw morto). Apostar?`,
      a: 'Sim — Ax sem showdown value e candidato a bluff no range polarizado do river',
      b: 'Não — Ace high ainda pode ganhar, check back',
      aCorrect: true,
      explanation: `Ax de naipe do flush draw que não fechou = mão sem valor de showdown na maioria dos casos. No range polarizado do river, essa mão e um bluff natural: não ganha no showdown mas pode forcar o vilão a foldar mãos medias. Sizing grande (66-100%) para maximizar fold equity.`,
  ...
  () => {
    ...
    return {
      ...
      b: 'Não — underpair e fraco demais, deve sempre checar',
      aCorrect: true,
      explanation: `Em estratégia merged com sizing pequeno (25-33%), underpairs podem fazer parte do range de aposta pois tem showdown value contra overcards do vilão. Não e uma aposta de valor pura, mas contribui para densidade de range. Se o vilão raise, você faz fold facil dado o sizing pequeno.`,
  ...
  () => {
    ...
    const pos = pickPos(IP_POSITIONS)
    return {
      q: `River ${ranks.join('-')}. Você e ${pos.heroPos} (IP) com top pair kicker ruim (ex: A2 em board com A). Apostar ou check?`,
      a: 'Check back — top pair kicker ruim e mão mediana no river polarizado: não e nuts nem bluff',
      b: 'Apostar 50% pot — ainda e top pair, tem valor',
      aCorrect: true,
      explanation: `No river IP com estratégia polarizada, top pair kicker ruim e uma mão "do meio". Não e forte o suficiente para valor (muitas mãos melhores chamam), mas tem showdown value demais para bluffar. Check back e a jogada correta — você ganha no showdown contra bluffs do vilão sem precisar apostar.`,
      ...
      content: (
        <div>
          ...
          <div className="rounded-lg p-4" style={{ background: 'rgba(79,206,130,0.08)', border: '1px solid rgba(79,206,130,0.2)' }}>
            ...
            <div style={{ color: '#b3b3b8', fontSize: 13, lineHeight: 1.7 }}>
              Aposta com <strong style={{ color: '#fdfdfd' }}>tudo que e bom</strong>, do melhor ate mãos medianas.<br/>
              Checa apenas o <strong style={{ color: '#fdfdfd' }}>lixo</strong> que não tem valor.
            </div>
          </div>
        </div>
      ),
      ...
      content: (
        <div>
          ...
          <div className="space-y-2 mb-4">
            {[
              { cond: 'IP (em posição)', why: 'Você ve a reacao do vilão antes de agir' },
              ...
              { cond: 'vilão com range capped', why: 'Ele checou 2x = range fraco, você polariza contra' },
            ].map((item, i) => (
          ...
          <div className="rounded-lg p-4" style={{ background: 'rgba(229,72,77,0.08)', border: '1px solid rgba(229,72,77,0.2)' }}>
            <div style={{ color: '#e5484d', fontSize: 13, fontWeight: 600 }}>
              Polarizado = sizing GRANDE (66-100%+) — valor máximo ou fold equity maxima
            </div>
          </div>
        </div>
      ),
    },
    {
      ...
      content: (
        <div>
          ...
          <div className="space-y-2 mb-4">
            {[
              { cond: 'OOP (fora de posição)', why: 'Sem info, precisa proteger range de check' },
              { cond: 'Flop (street inicial)', why: 'Ranges ainda amplos, não da pra polarizar bem' },
              { cond: 'Boards secos', why: 'Poucas draws, equities estaveis' },
              { cond: 'Range advantage claro', why: 'Apostar com muitas mãos e lucrativo com sizing pequeno' },
            ].map((item, i) => (
              <div key={i} className="rounded-lg px-3 py-2.5" style={{ background: '#222225' }}>
          ...
          <div className="rounded-lg p-4" style={{ background: 'rgba(79,206,130,0.08)', border: '1px solid rgba(79,206,130,0.2)' }}>
            <div style={{ color: '#4fce82', fontSize: 13, fontWeight: 600 }}>
              Merged = sizing PEQUENO (25-50%) — muitas mãos de valor, não precisa de fold equity
            </div>
          </div>
        </div>
      ),
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {[
                  ...
                  ['Checa', 'Mãos medianas', 'Lixo'],
                  ['Sizing', '66-100%+', '25-50%'],
                  ['Posição', 'IP preferido', 'OOP preferido'],
