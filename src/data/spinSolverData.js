/**
 * SPIN & GO SOLVER DATA — Multi-Source Pre-Computed Ranges
 *
 * Fontes:
 * 1. Spin_and_Go_Solver (jamdickin11) — MCCFR 15bb 3-max 7% rake (12 action histories)
 * 2. Nash Push/Fold HU — Jennifear/ICMIZER/HRC/poker-cfr/CandyPoker consensus (7 stacks)
 *
 * Ranges MCCFR: fold sets invertidos para play sets (169 mãos completas)
 * Notação convertida de C++ ("810o" → "T8o", "3Ao" → "A3o", "10Qs" → "QTs")
 *
 * Action History (3-max):
...
// ═══════════════════════════════════════════════════════════
// HU PUSH/FOLD NASH EQUILIBRIUM — Multi-Stack
// Fontes: Jennifear charts, ICMIZER, HRC, poker-cfr (b-inary),
// CandyPoker (sweeterthancandy), PokerStars Nash tables
//
// HU Push/Fold: SB (BTN) decide push ou fold, BB decide call ou fold
// Estas tabelas são consenso matemático — todos os solvers convergem
// para os mesmos resultados (variação <1-2% entre eles)
// ═══════════════════════════════════════════════════════════

const HU_NASH_PUSH = {
  // SB push ranges por stack (mãos que devem ir all-in)
  5: new Set([
    'AA','KK','QQ','JJ','TT','99','88','77','66','55','44','33','22',
    'AKs','AQs','AJs','ATs','A9s','A8s','A7s','A6s','A5s','A4s','A3s','A2s',
    'KQs','KJs','KTs','K9s','K8s','K7s','K6s','K5s','K4s','K3s','K2s',
...
const HU_NASH_CALL = {
  // BB call ranges vs SB shove por stack
  5: new Set([
    'AA','KK','QQ','JJ','TT','99','88','77','66','55','44','33','22',
    'AKs','AQs','AJs','ATs','A9s','A8s','A7s','A6s','A5s','A4s','A3s','A2s',
    'KQs','KJs','KTs','K9s','K8s','K7s','K6s','K5s','K4s','K3s','K2s',
...
function getHuNashTier(stackBB) {
  if (stackBB <= 6) return 5
  if (stackBB <= 8) return 7
  if (stackBB <= 11) return 10
  if (stackBB <= 14) return 13
...
/**
 * Lookup Nash push/fold HU para a fase HU do Spin & Go.
 * @param {string} hand - Notação PA (ex: "AKs", "T9o", "JJ")
 * @param {string} position - "SB" (pusher) ou "BB" (caller)
 * @param {number} stackBB - Stack em big blinds
 * @param {string} action - "push"/"allin"/"raise" ou "call" ou "fold"
 */
export function lookupHuNash(hand, position, stackBB) {
  const tier = getHuNashTier(stackBB)

  if (position === 'SB' || position === 'BTN') {
    const pushSet = HU_NASH_PUSH[tier]
    if (!pushSet) return null
    const shouldPush = pushSet.has(hand)
    return {
      shouldPlay: shouldPush,
      shouldFold: !shouldPush,
      action: 'push',
      rangeDescription: `SB push ${tier}bb HU Nash`,
      rangePct: Math.round((pushSet.size / 169) * 100),
      confidence: 1.0,
      source: `Nash Push/Fold HU (${tier}bb) — Jennifear/ICMIZER/HRC/poker-cfr consensus`,
    }
  }

  if (position === 'BB') {
    const callSet = HU_NASH_CALL[tier]
    if (!callSet) return null
    const shouldCall = callSet.has(hand)
    return {
      shouldPlay: shouldCall,
      shouldFold: !shouldCall,
      action: 'call',
      rangeDescription: `BB call vs push ${tier}bb HU Nash`,
      rangePct: Math.round((callSet.size / 169) * 100),
      confidence: 1.0,
      source: `Nash Push/Fold HU (${tier}bb) — Jennifear/ICMIZER/HRC/poker-cfr consensus`,
    }
  }
